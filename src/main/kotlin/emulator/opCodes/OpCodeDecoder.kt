...
class OpCodeDecoder(private val cpu: CPU, private val mmu: MMU, private val ppu: PPU, private val debugWriter: debugWriter) {

    //Arithmetic Logic Unit Block Codes
    private val adc = adc_OpCodes(cpu, ppu, mmu, debugWriter)
    private val and = and_OpCodes(cpu, mmu, debugWriter)
    private val cmp = cmp_OpCodes(cpu, mmu, debugWriter)
    private val eor = eor_OpCodes(cpu, mmu, debugWriter)
    private val lda = lda_OpCodes(cpu, mmu, debugWriter)
    private val ora = ora_OpCodes(cpu, mmu, debugWriter)
    private val sbc = sbc_OpCodes(cpu, mmu, debugWriter)
    private val sta = sta_OpCodes(cpu, mmu, debugWriter)

    //Control Block Codes
    private val bcc = bcc_OpCodes(cpu, mmu, debugWriter)
    private val bcs = bcs_OpCodes(cpu, mmu, debugWriter)
    private val beq = beq_OpCodes(cpu, mmu, debugWriter)
    private val bit = bit_OpCodes(cpu, mmu, debugWriter)
    private val bmi = bmi_OpCodes(cpu, mmu, debugWriter)
    private val bne = bne_OpCodes(cpu, mmu, debugWriter)
    private val bpl = bpl_OpCodes(cpu, mmu, debugWriter)
    private val brk = brk_OpCodes(cpu, mmu, debugWriter)
    private val bvc = bvc_OpCodes(cpu, mmu, debugWriter)
    private val bvs = bvs_OpCodes(cpu, mmu, debugWriter)
    private val clc = clc_OpCodes(cpu, debugWriter)
    private val cld = cld_OpCodes(cpu, debugWriter)
    private val cli = cli_OpCodes(cpu, debugWriter)
    private val clv = clv_OpCodes(cpu, debugWriter)
    private val jmp = jmp_OpCodes(cpu, mmu, ppu, debugWriter)
    private val jsr = jsr_OpCodes(cpu, mmu, debugWriter)
    private val nop = nop_OpCodes(cpu, debugWriter)
    private val pha = pha_OpCodes(cpu, mmu, debugWriter)
    private val php = php_OpCodes(cpu, mmu, debugWriter)
    private val pla = pla_OpCodes(cpu, mmu, debugWriter)
    private val plp = plp_OpCodes(cpu, mmu, debugWriter)
    private val rti = rti_OpCodes(cpu, mmu, debugWriter)
    private val rts = rts_OpCodes(cpu, mmu, debugWriter)
    private val sec = sec_OpCodes(cpu, debugWriter)
    private val sed = sed_OpCodes(cpu, debugWriter)
    private val sei = sei_OpCodes(cpu, debugWriter)
    private val tay = tay_OpCodes(cpu, debugWriter)
    private val tya = tya_OpCodes(cpu, debugWriter)

    //Read-Modify-Write Block Codes
    private val asl = asl_OpCodes(cpu, mmu, debugWriter)
    private val cpx = cpx_OpCodes(cpu, mmu, debugWriter)
    private val cpy = cpy_OpCodes(cpu, mmu, debugWriter)
    private val dec = dec_OpCodes(cpu, mmu, debugWriter)
    private val inc = inc_OpCodes(cpu, mmu, debugWriter)
    private val ldx = ldx_OpCodes(cpu, mmu, debugWriter)
    private val ldy = ldy_OpCodes(cpu, mmu, debugWriter)
    private val lsr = lsr_OpCodes(cpu, mmu, debugWriter)
    private val rol = rol_OpCodes(cpu, mmu, debugWriter)
    private val ror = ror_OpCodes(cpu, mmu, debugWriter)
    private val stx = stx_OpCodes(cpu, mmu, debugWriter)
    private val sty = sty_OpCodes(cpu, mmu, debugWriter)

    //Unofficial Block Codes
    private val ahx = ahx_OpCodes(cpu, mmu, debugWriter)
    private val alr = alr_OpCodes(cpu, mmu, debugWriter)
    private val anc = anc_OpCodes(cpu, mmu, debugWriter)
    private val arr = arr_OpCodes(cpu, mmu, debugWriter)
    private val axs = axs_OpCodes(cpu, mmu, debugWriter)
    private val dcp = dcp_OpCodes(cpu, mmu, debugWriter)
    private val isc = isc_OpCodes(cpu, mmu, debugWriter)
    private val las = las_OpCodes(cpu, mmu, debugWriter)
    private val lax = lax_OpCodes(cpu, mmu, debugWriter)
    private val rla = rla_OpCodes(cpu, mmu, debugWriter)
    private val rra = rra_OpCodes(cpu, mmu, debugWriter)
    private val sax = sax_OpCodes(cpu, mmu, debugWriter)
    private val shx = shx_OpCodes(cpu, mmu, debugWriter)
    private val shy = shy_OpCodes(cpu, mmu, debugWriter)
    private val slo = slo_OpCodes(cpu, mmu, debugWriter)
    private val sre = sre_OpCodes(cpu, mmu, debugWriter)
    private val tas = tas_OpCodes(cpu, mmu, debugWriter)
    private val xaa = xaa_OpCodes(cpu, mmu, debugWriter)
